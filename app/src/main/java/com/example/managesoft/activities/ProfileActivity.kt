...
import com.example.managesoft.databinding.ActivityProfileBinding
...
class ProfileActivity : BaseActivity() {
    ...
    fun updateUserProfileData(){
        ...
        if (binding?.upName?.text.toString() != mUserDetails.name){
            userHashMap[Constants.NAME] = binding?.upName?.text.toString()
        }

        if (binding?.upMobile?.text.toString() != mUserDetails.mobile.toString()){
            userHashMap[Constants.MOBILE] = binding?.upMobile?.text.toString().toLong()
        }

        FirestoreClass().updateUserProfileData(this,userHashMap)
    }

    fun setMyProfileUserDetails(user : User){

        mUserDetails = user

        Glide.with(this).load(user.image).centerCrop()
            .placeholder(R.drawable.ic_user_place_holder)
            .into(findViewById(R.id.up_image))
        binding?.upName?.setText(user.name)
        binding?.upEmail?.setText(user.email)
        binding?.upMobile?.setText(user.mobile.toString())
    ...
    private fun uploadUserImage(){
        ...
        if (mSelectedImageFileUri != null){
            ...
            getInstance().reference.child("USER_IMAGE" +
